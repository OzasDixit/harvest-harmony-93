        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="mb-1 flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-gold">
              <PackageCheck className="size-4 text-gold" /> {t("sellVsStore.decisionMatrix")}
            </div>
            <h2 className="text-xl font-bold tracking-tight text-ink sm:text-2xl">
              {t("sellVsStore.title")}
            </h2>
            <p className="mt-1 text-sm text-mute">
              {t("sellVsStore.subtitle")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-md bg-panel p-4 ring-1 ring-line">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[30, 60, 90].map((days) => (
            ...
            >
              <div className="text-sm font-medium">{t("sellVsStore.daysHold", { days })}</div>
              <div className="text-xs text-leaf mt-1">
                {t("sellVsStore.expectedIncrease", { amount: days === 30 ? "470" : days === 60 ? "640" : "720" })}
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="rounded-md bg-panel p-5 ring-1 ring-line flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-line pb-3">
              <span className="text-xs font-semibold uppercase tracking-wider text-mute">{t("sellVsStore.optionA")}</span>
              <span className="rounded bg-panel2 px-2 py-0.5 text-xs text-mute">{t("sellVsStore.immediateLiquidity")}</span>
            </div>
            <h3 className="mt-3 text-lg font-bold text-ink">{t("sellVsStore.sellTodayMandi")}</h3>
            <div className="mt-4 space-y-2 text-xs text-mute">
              <div className="flex justify-between">
                <span>{t("sellVsStore.currentMandiPrice")}</span>
                <span className="font-mono text-ink">₹{currentMandiPrice} / quintal</span>
              </div>
              <div className="flex justify-between">
                <span>{t("sellVsStore.totalVolume")}</span>
                <span className="font-mono text-ink">122 Quintals (12.2 t)</span>
              </div>
              <div className="flex justify-between">
                <span>{t("sellVsStore.storageSpoilageCost")}</span>
          ...
          <div className="mt-6 rounded-md bg-panel2 p-3.5 ring-1 ring-line">
            <div className="text-xs text-faint uppercase">{t("sellVsStore.netRevenueToday")}</div>
        ...
        <div className="rounded-md bg-panel p-5 ring-1 ring-leaf/40 border-2 border-leaf/30 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-line pb-3">
              <span className="text-xs font-semibold uppercase tracking-wider text-leaf flex items-center gap-1">
                <ShieldCheck className="size-3.5" /> {t("sellVsStore.optionB")}
              </span>
              <span className="rounded bg-leaf/20 px-2 py-0.5 text-xs font-semibold text-leaf">
                {t("sellVsStore.higherGain", { percent: Math.round((netAdvantage / sellNowRevenue) * 100) })}
              </span>
            </div>
            <h3 className="mt-3 text-lg font-bold text-leaf">{t("sellVsStore.storeForDays", { days: holdingDays })}</h3>
            <div className="mt-4 space-y-2 text-xs text-mute">
              <div className="flex justify-between">
                <span>{t("sellVsStore.projectedMandiPrice")}</span>
                <span className="font-mono text-leaf font-medium">₹{expectedPricePerQuintal} / quintal</span>
              </div>
              <div className="flex justify-between">
                <span>{t("sellVsStore.storageRental", { days: holdingDays })}</span>
                <span className="font-mono text-risk">-₹{totalStorageCost.toLocaleString("en-IN")}</span>
              </div>
              <div className="flex justify-between">
                <span>{t("sellVsStore.spoilageBuffer", { percent: (0.02 * months * 100).toFixed(1) })}</span>
                <span className="font-mono text-mute">{t("sellVsStore.netVolume", { volume: netSellableTonnes.toFixed(1) })}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-md bg-leaf/10 p-3.5 ring-1 ring-leaf/30">
            <div className="flex items-center justify-between">
              <span className="text-xs text-faint uppercase">{t("sellVsStore.netRevenueAfterStorage")}</span>
              <span className="text-xs font-bold text-leaf">{t("sellVsStore.advantage", { amount: netAdvantage.toLocaleString("en-IN") })}</span>
            </div>
