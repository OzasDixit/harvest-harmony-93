import { Gauge, Download, ArrowUpRight } from "lucide-react";
...
export function ProfitView({ notify }: ProfitViewProps) {
  ...
  const costBreakdownData = [
    { name: t("profit.seedFertilizer"), value: financialSummary.seedFertilizerCost, color: "oklch(0.68 0.17 145)" },
    { name: t("profit.labourTillage"), value: financialSummary.labourCost, color: "oklch(0.78 0.14 82)" },
    { name: t("profit.coldStorage"), value: financialSummary.storageCost, color: "oklch(0.71 0.12 194)" },
    { name: t("profit.irrigationDrip"), value: financialSummary.irrigationCost, color: "oklch(0.69 0.16 43)" },
    { name: t("profit.logistics"), value: financialSummary.logisticsCost, color: "oklch(0.47 0.035 145)" },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="mb-1 flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-gold">
              <Gauge className="size-4 text-gold" /> {t("profit.cockpit")}
            </div>
          ...
          <button type="button" onClick={exportFinancials} className="action-primary text-xs">
            <Download className="size-3.5" /> {t("profit.downloadReport")}
          </button>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-md bg-panel p-4 ring-1 ring-line">
          <div className="text-xs text-faint uppercase tracking-wider">{t("profit.grossRevenue")}</div>
        ...
        <div className="rounded-md bg-panel p-4 ring-1 ring-line">
          <div className="text-xs text-faint uppercase tracking-wider">{t("profit.totalExpenses")}</div>
        ...
        <div className="rounded-md bg-panel p-4 ring-1 ring-leaf/40 bg-leaf/5">
          <div className="flex items-center justify-between">
            <span className="text-xs text-faint uppercase tracking-wider">{t("profit.netProfit")}</span>
            <span className="rounded bg-leaf/20 px-2 py-0.5 text-xs font-bold text-leaf">{t("profit.roi", { percent: roiMargin })}</span>
          </div>
          ...
          <div className="mt-1 text-xs text-leaf font-medium flex items-center gap-1">
            <ArrowUpRight className="size-3" /> {t("profit.netGainFromStorage")}
          </div>
        </div>
      </div>
